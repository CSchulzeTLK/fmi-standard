...
_If an optional element is present and defines a list (such as `<UnitDefinitions>`), the list must have at least one element (such as `<Unit>`).]_
...
The details of these elements are defined in <<ModelExchange>> and <<basic-co-simulation>>.
...
image::images/fmiModelDescription_UnitDefinitions_Schema.png[]
image::images/fmiModelDescription_UnitDefinitions_Unit_Schema.png[]

It consists of zero or more `Unit` definitions.
_[If no units are defined, element `<UnitDefinitions>` must not be present._
_If 1 or more units are defined, this element must be present.]_
...
If a variable is associated with a `Unit`, then the value of the variable has to be provided with the `fmi3Set{VariableType}` functions or else is returned by the `fmi3Get{VariableType}` functions with respect to this `Unit`.
...
image::images/fmi3Unit_BaseUnit_Schema.png[width=50%, align="center"]
...
image::images/fmi3Unit_DisplayUnit_Schema.png[width=80%, align="center"]
...
//TODO: Cannot find SimpleType
image::images/fmiModelDescription_TypeDefinitions_Schema.png[width=90%, align="center"]
//image::images/fmi3TypeDefinitionBase_Schema.png[width=70%, align="center"]
...
image::images/fmi3VariableBase_Schema.png[width=80%, align="center"]

image::images/fmi3VariableBase_Dimension_Schema.png[width=50%, align="center"]

image::images/fmi3VariableBase_Annotations_Schema.png[width=50%, align="center"]

image::images/fmi3Float32_Schema.png[width=50%, align="center"]

image::images/fmi3Float64_Schema.png[[width=50%, align="center"]]

image::images/fmi3Int8_Schema.png[width=50%, align="center"]

image::images/fmi3UInt8_Schema.png[width=50%, align="center"]

image::images/fmi3Int16_Schema.png[width=50%, align="center"]

image::images/fmi3UInt16_Schema.png[width=50%, align="center"]

image::images/fmi3Int32_Schema.png[width=50%, align="center"]

image::images/fmi3UInt32_Schema.png[width=50%, align="center"]

image::images/fmi3Int64_Schema.png[width=50%, align="center"]

image::images/fmi3UInt64_Schema.png[width=50%, align="center"]

image::images/fmi3UInt64_Schema.png[width=50%, align="center"]

image::images/fmi3Boolean_Schema.png[width=80%, align="center"]

image::images/fmi3String_Schema.png[width=80%, align="center"]

image::images/fmi3Binary_Schema.png[width=80%, align="center"]

image::images/fmi3Enumeration_Schema.png[width=80%, align="center"]

image::images/fmi3Clock_Schema.png[width=80%, align="center"]
...
_If an FMU defines min/max values for integer types and Enumerations (<<local>> and <<output>> variables), then the expected behavior of the FMU is that `fmi3Get{VariableType}` functions return values in the defined range._
...
image::images/fmiModelDescription_LogCategories_Schema.png[width=70%, align="center"]
image::images/fmiModelDescription_LogCategories_Category_Schema.png[width=70%, align="center"]
...
image::images/fmiModelDescription_DefaultExperiment_Schema.png[width=70%, align="center"]
...
image::images/fmi3Annotation_Schema.png[width=80%, align="center"]
image::images/fmi3Annotation_Tool_Schema.png[width=80%, align="center"]
...
image::images/fmiModelDescription_ModelVariables_Schema.png[width=100%, align="center"]
...
_For example, <<discrete>> variables change their values only at event instants (Model Exchange) or at a communication point (Co-Simulation) and it is therefore only necessary to inquire them with `fmi3Get{VariableType}` and store them at event times.]_
Allowed values of this enumeration:
...
_WARNING: It is not recommended to change the start values in the `modelDescription.xml` file of an FMU, as this would break the consistency with the hard-coded start values in the C-Code._
_This could lead to unpredictable behavior of the FMU in different importing tools, as it is not mandatory to call `fmi3Set{VariableType}` to set start values during initialization. Instead it is recommended to use the SSP Standard (https://ssp-standard.org/) to handle modified parameters of FMUs or different parameter sets._
...
Since, <<output,`outputs`>>, continuous-time <<state,`states`>> and state derivatives are either present as `Knowns` (if <<initial>> = <<exact>>) or as <<Unknown,`Unknowns`>> (if <<initial>> = <<approx>> or <<calculated>>), they can be inquired with `fmi3Get{VariableType}` in *Initialization Mode*.
...
_[If, for example, a continuous-time <<output>>_ latexmath:[\color{blue}{y_{2}}] _is a function of the continuous-time <<input,`inputs`>>_ latexmath:[\color{blue}{u_{3}}] _and_ latexmath:[\color{blue}{u_{5}}], _and these <<input,`inputs`>> have changed, then_ `fmi3Set{VariableType}` _on_ latexmath:[\color{blue}{u_{3}}] _and_ latexmath:[\color{blue}{u_{5}}] _must always be called before calling_ `fmi3Get{VariableType}` _on_ latexmath:[\color{blue}{y_{2}}] _.]_
...
`Knowns` latexmath:[\color{blue}{\mathbf{v}_{\text{known}}}] in *Event Mode* and *Continuous-Time Mode* (Model Exchange) and at communication points (Co-Simulation) for elements `Outputs`, `Derivatives`:
...
Only for floating point type unknowns latexmath:[\color{blue}{v_{\text{unknown}}}] in Event and *Continuous-Time Mode* (Model Exchange) and at communication points (Co-Simulation), and not for <<InitialUnknowns>> for *Initialization Mode*:
...
With attribute `variableNamingConvention` of element `fmiModelDescription`, the convention is defined how the variable names have been constructed.
